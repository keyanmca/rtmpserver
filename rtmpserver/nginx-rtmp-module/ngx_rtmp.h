...
/*
 * 整个结构就是一个rtmp的chunk
 * 其中in是个链表节点，buf存储当前收到的数据，next表示该chunk stream id下收到的所有stream形成的链表
 */
typedef struct {
    ngx_rtmp_header_t       hdr; // chunk header
    uint32_t                dtime; // timestamp
    uint32_t                len; /* 当前已接收到的chunk包总长度 */
    uint8_t                 ext; // extened timestamp
    ngx_chain_t            *in;
} ngx_rtmp_stream_t;
...
/*
 * 该数据结构用于rtmp的交互，一个连接对应一个session
 */
typedef struct {
    ...
    /*
     * 存储各个配置支持功能的数据，并且每个命令有一个全局变量结构，
     * 譬如: live对应于ngx_rtmp_live_module全局变量，在函数ngx_rtmp_init_session中初始化
     */
    void                  **ctx;
    
    void                  **main_conf; //main的配置
    void                  **srv_conf; // server的配置
    void                  **app_conf; // app属性的配置
    ...
    /* connection parameters connect命令的参数 */
    ngx_str_t               app;
    ngx_str_t               args;
    ngx_str_t               flashver;
    ngx_str_t               swf_url;
    ...
    u_char                 *hs_digest;
    unsigned                hs_old:1;
    ngx_uint_t              hs_stage; // 握手的状态机, 定义起始值为: NGX_RTMP_HANDSHAKE_SERVER_RECV_CHALLENGE
    ...
    ngx_rtmp_stream_t      *in_streams; // 收到的stream信息，是个数组，数组长度为配置中max_streams的值，默认为32
    uint32_t                in_csid; // chunk stream id
    ngx_uint_t              in_chunk_size; // 收到的chunk size
    ngx_pool_t             *in_pool;
    uint32_t                in_bytes;
...
typedef struct ngx_rtmp_core_srv_conf_s {
    ...
    ngx_int_t               max_streams; // 支持最大的stream数，默认为32
