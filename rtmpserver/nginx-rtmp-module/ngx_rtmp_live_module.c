...
ngx_rtmp_live_get_stream(ngx_rtmp_session_t *s, u_char *name, int create)
{
    ...
    // 先查找stream，看是否已经加入了，publisher与subscriber用这个进行关联
    // 当subscriber访问时，会查看stream信息，若查到后直接return
    len = ngx_strlen(name);
    stream = &lacf->streams[ngx_hash_key(name, len) % lacf->nbuckets];

    // 若已经存在相同名字的stream，则找到后直接返回
    ...
    // 创建一个新的stream，先从free链表中找，若没有再分配内存
...
ngx_rtmp_live_start(ngx_rtmp_session_t *s)
{
    ...
    control = ngx_rtmp_create_stream_begin(s, NGX_RTMP_MSID); // 发送stream begin的消息
...
ngx_rtmp_live_join(ngx_rtmp_session_t *s, u_char *name, unsigned publisher)
{
    ...
    // 新加入的live直播，分配内存创建新的ctx，并写回live的全局变量中
    if (ctx == NULL) {
    ...
    ctx->session = s; // 对应的session信息

    ngx_log_debug1(NGX_LOG_DEBUG_RTMP, s->connection->log, 0,
                   "live: join '%s'", name);

    // stream是直接来源于nginx配置数据结构中的，所以可以被各个work共享
    ...
    ctx->stream = *stream; // 保存当前上下文的stream
    ctx->publishing = publisher; // 标记是否为publisher
    ctx->next = (*stream)->ctx; // 把订阅同一个stream的用ctx链表链起来，头插法

    (*stream)->ctx = ctx; // 保存新的头节点

    if (lacf->buflen) {
        s->out_buffer = 1;
    }

    // 音视频使用默认的chunk stream id
    ctx->cs[0].csid = NGX_RTMP_CSID_VIDEO; 
    ctx->cs[1].csid = NGX_RTMP_CSID_AUDIO;
    ctx->cs[2].csid = NGX_RTMP_CSID_CHAT;
...
// add by gw
static ngx_int_t
ngx_rtmp_live_chat(ngx_rtmp_session_t *s, ngx_rtmp_header_t *h,
                 ngx_chain_t *in)
{
    ngx_rtmp_live_ctx_t            *ctx, *pctx;    
    ngx_chain_t                    *header, *coheader, *rpkt;
    ngx_rtmp_core_srv_conf_t       *cscf;
    ngx_rtmp_live_app_conf_t       *lacf;
    ngx_rtmp_session_t             *ss;
    ngx_rtmp_header_t               ch, lh, clh;
    ...
    for (pctx = ctx->stream->ctx; pctx; pctx = pctx->next) {
        ...
        if (ngx_rtmp_send_message(ss, rpkt, 0) != NGX_OK) {
            ++pctx->ndropped;
            cs->dropped += delta;            
            continue;
        }
...
ngx_rtmp_live_postconfiguration(ngx_conf_t *cf)
{
    ...
    *h = ngx_rtmp_live_av;
    ...
    h = ngx_array_push(&cmcf->events[NGX_RTMP_MSG_CHAT]);
    *h = ngx_rtmp_live_chat;
