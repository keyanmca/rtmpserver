...
ngx_rtmp_bandwidth_t        ngx_rtmp_bw_out; // 发送的rtmp数据，当达到bandwidth大小后，需要对端回复ack
ngx_rtmp_bandwidth_t        ngx_rtmp_bw_in; // 接收到的rtmp数据，当达到bandwidth大小后，需要回复ack
...
ngx_rtmp_recv(ngx_event_t *rev)
{
    ...
    /*
     * 接收、解析chunk，组织message
     * 1. 申请接收chunk的内存
     * 2. 接收chunk数据
     * 3. 解析chunk
     * 4. 组织message，若未完成，继续接收新的chunk
     */
    for( ;; ) {
        
        /*
         * 1. 申请接收chunk的内存
         * in_streams其数据结构用于存储收到的stream信息，是个数组，下标即为chunk stream id, 数组长度为配置中max_streams的值，默认为32
         * 对于chunk stream id，协议规定0, 1, 2做预留，不会被使用。故代码中用in_streams[0]做了个小技巧，每次来了新chunk数据，都分配内存
         * 挂在in_streams[0]上，然后再赋给in_streams[真正的chunk stream id]
         */
        st = &s->in_streams[s->in_csid]; // s->in_csid每次都为0
        ...
        if (old_size) {
            ...
            s->ping_reset = 1;
            ngx_rtmp_update_bandwidth(&ngx_rtmp_bw_in, n); // 更新接收到的数据量
            b->last += n;
            s->in_bytes += n;
        ...
        // 3. 解析chunk; 包括chunk basic header、message header、extended timestamp
        /* parse headers */
        if (b->pos == b->start) {
            p = b->pos;
            ...
            if (csid == 0) {
                if (b->last - p < 1) // 第一个字节中的后6位为0，表示chunk basic header长度为2个字节，故此时数据未读完
                    continue;
                csid = 64;
                csid += *(uint8_t*)p++;

            } else if (csid == 1) { // 第一个字节中的后6位为1，表示chunk basic header长度为3个字节
                if (b->last - p < 2)
                    continue;
                csid = 64;
                csid += *(uint8_t*)p++;
            ...
            if (s->in_csid == 0) {
                ...
                st->in = st->in->next; // 把in_streams[0].in设置成NULL

                /* link to new stream */
                s->in_csid = csid; // 关联chunk stream id

                st = &s->in_streams[csid]; // 取出真正的chunk stream id对应的数据结构
                if (st->in == NULL) {
                    in->next = in;
                } else {
                    in->next = st->in->next;
                ...
                st->in = in; // 将节点指针赋给in_streams[真正的chunk stream id]
                h = &st->hdr;
                h->csid = csid; // 将chunk stream id记录到节点中
            }

            // 根据fmt类型，处理message header和extended timestamp
            ext = st->ext;
            timestamp = st->dtime;
            if (fmt <= 2 ) {
                if (b->last - p < 3)
        ...
        size = b->last - b->pos; // 计算剩余未解析数据的长度
        fsize = h->mlen - st->len; // 计算message未收到的剩余数据长度

        if (size < ngx_min(fsize, s->in_chunk_size)) // message还没有收完，继续
            continue;

        /* buffer is ready */

        if (fsize > s->in_chunk_size) {
            /* collect fragmented chunks 继续接收message剩余部分 */
        ...
        s->in_csid = 0; // 把s->in_csid清0，为下一次的分配内存做准备
    }
}
        ...
        ngx_rtmp_header_t *h, ngx_chain_t *in)
{
    ...
    evhs = &cmcf->events[h->type]; // 回调handle在ngx_rtmp_init_event_handlers函数中注册
    evh = evhs->elts;
